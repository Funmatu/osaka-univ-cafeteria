...
**データソース**: [大阪大学生協 豊中図書館下食堂 (west2-univ.jp, t=663252)](https://west2-univ.jp/sp/index.php?t=663252)

日替わりメニューから、**予算内で栄養バランスの良い組合せ (A/B/C 3 定食)** を自動提案する純粋静的 Web アプリ。
...
- 毎朝 06:00 JST に GitHub Actions が west2-univ.jp から最新メニューと**本物の栄養成分**を取得、データが変化していれば自動 commit & 再デプロイ。
...
│       食物繊維・B1・B2 の欠落を補填                     │
│    ④ public/data/menu.json + meta.json を上書き        │
...
│  GitHub Pages (純粋静的配信)                            │
│                                                        │
│  index.html → fetch('./data/menu.json')                │
...
│   ├── index.html
│   ├── js/
│   │   ├── app.js               # UI オーケストレーション
│   │   ├── optimizer.js         # 組合せ列挙 + スコアリング呼び出し
│   │   ├── nutrition.js         # RDI 目標 + スコア関数 (純関数)
│   │   ├── combo-rules.js       # カテゴリ制約 (定食/一品)
│   │   └── diversity.js         # MMR 多様化 + Boltzmann サンプラ
│   ├── css/styles.css
│   └── data/
│       ├── menu.json            # スクレイパ出力 (自動更新・commit 対象)
│       └── meta.json            # 更新日時・店舗情報・参照 DB メタ
│
├── test/                        # Node.js built-in test runner
│   ├── nutrition.test.mjs       # スコア関数の境界値テスト
...
| URL | 内容 | レンダリング | 取得方法 |
|---|---|---|---|
| `sp/index.php?t=663252` | 注目4品 | 静的 HTML | (未使用) |
| `sp/menu.php?t=663252` | カテゴリ別メニュー (ビュー) | **JS レンダリング** | (未使用) |
| `sp/menu_load.php?t=663252&a=on_X` | カテゴリ別アイテム HTML 片 (Ajax 応答) | **静的 HTML** | ✓ メイン取得 |
| `sp/detail.php?t=663252&c=XXXXXX` | アイテム個別詳細+栄養成分 | 静的 HTML | ✓ 栄養抽出用 |
...
2. `enrichWithDetail()`: 47 件の `detail.php` を **並列度 4 / 各 500ms スリープ** で取得、[`scripts/parser.mjs`](scripts/parser.mjs) `parseDetailHtml()` で栄養成分抽出
...
5. code 順でソート、`public/data/menu.json` と `meta.json` に書き出し
6. GitHub Actions 側で `git diff --staged --quiet` を判定、差分があれば auto-commit
...
```json
{
  ...
  "lastUpdated": "2026-04-24T08:06:40.439Z",
  "itemCount": 47,
  "withNutrition": 46,
  "augmented": 47,
  "issues": 1,
...
| 名前が全部「大阪大学生協豊中図書館下食堂」 | detail.php の h1 構造変更 | [`scripts/parser.mjs`](scripts/parser.mjs) `SITE_HEADER_NAMES` 更新 |
...
west2-univ.jp は **パラメータ `t=XXXXXX`** でカフェテリアを指定する構造。大阪大学生協は豊中・吹田・箕面の各キャンパスに複数食堂があり、いずれも同じ URL 形態で取得可能:

- 豊中図書館下食堂 (現行、`t=663252`)
- 豊中福利センター食堂
- 吹田セルフカフェテリア (理系キャンパス)
- 吹田宙 (理系)
- 箕面食堂
- その他 (らふぉれ、DONSUCA、Cafe などを含む)

**必要な変更は最小限**:
1. [`scripts/scrape.mjs`](scripts/scrape.mjs) の `CAFETERIA_ID` を配列化し、全食堂分ループ取得
2. `public/data/{cafeteria-id}/menu.json` のようにディレクトリ分割して保存
3. UI ([`public/index.html`](public/index.html), [`public/js/app.js`](public/js/app.js)) にタブ UI を追加し、食堂切替で読込先 JSON を切替
4. `meta.json` を食堂リスト込みに拡張

**設計意図**: 他大学への拡張ではなく、**大阪大学生協内の横断比較**を目的とする (あの食堂ならこの定食が安い・栄養良い、など)。
...
| 項目 | 出典 |
|---|---|
| メニュー情報・基本栄養素 | [west2-univ.jp (大学生協事業連合)](https://west2-univ.jp/sp/menu.php?t=663252) © 大阪大学生協 |
